...
def _derive(root: Path, request_path: Path, request: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, raw in enumerate(request["owner_artifacts"]):
        ...
        accepted_schemas = ROLE_SCHEMAS[role]
        expected_schema = document["schema"]
        rw.require(expected_schema in accepted_schemas, f"role {role} cannot consume owner schema {expected_schema}")
    ...
    if "minimum_evidence" not in roles:
        ...
        minimum_source = roles["minimum_evidence"]["source_id"]
        blockers.extend([
            _blocker(
                "minimum_candidate_input_result_lineage_unavailable_v2", "minimum_evidence", [minimum_source],
                "Maturity gate /2 replays, but its minimum candidate-to-input-to-result lineage is explicitly unavailable.",
                "owner-validated exact minimum candidate, input and result lineage",
            ),
            _blocker(
                "exact_owner_ts_mode_artifact_v2_required", "ts_evidence", [minimum_source],
                "Maturity gate /2 does not substitute for the exact owner TS-mode artifact required by the protected action contract.",
                "exact owner TS-mode artifact /2",
            ),
            _blocker(
                "complete_owner_thermochemistry_evidence_v2_required", "quantity_lineage", [minimum_source],
                "Maturity gate /2 does not provide complete owner thermochemistry evidence for formal comparison or barrier reporting.",
                "complete owner thermochemistry evidence /2",
            ),
        ])
