...
def _assemble_nodes(
    ...
    precedent_blocker_id = "ts_precedent_validation_unavailable" if precedent_bound else "ts_precedent_map_missing"
    precedent_description = (
        f"Bound {PRECEDENT_SCHEMA} remains provenance-only because its owner schema and promotion validator are unavailable; presence cannot promote any edge-targeted node."
        if precedent_bound
        else f"Required {PRECEDENT_SCHEMA} is not bound; every edge-targeted node remains blocked."
    )
    global_blockers.append(_blocker(precedent_blocker_id, "study", precedent_description, ("edge_targeted_nodes", "scientific_readiness")))
...
def build_plan(
    ...
    if precedent_path is not None:
        ...
        _check_study_and_safety(precedent, review["study_id"], "TS precedent map")
    superseded_bindings: list[dict[str, Any]] = []
...
def _validate_plan_internal(path: Path, stack: set[Path]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        if precedent_binding is not None:
            precedent, _ = _resolve_binding(precedent_binding, path, PRECEDENT_SCHEMA, "TS precedent map")
            _check_study_and_safety(precedent, artifact["study_id"], "TS precedent map")
...
def _derive_index(plan: dict[str, Any], plan_binding: dict[str, Any], chain: dict[str, Any]) -> dict[str, Any]:
    ...
    artifacts: list[dict[str, Any]] = [
        ...
        {"role": "ts_precedent_map", "status": "missing" if plan["ts_precedent_map"] is None else "bound_unvalidated", "artifact": plan["ts_precedent_map"]},
    ...
    precedent_blocker_id = "ts_precedent_map_missing" if plan["ts_precedent_map"] is None else "ts_precedent_validation_unavailable"
    ...
    stage_specs.append(("ts_precedent_map", "ts_precedent_map", "missing" if plan["ts_precedent_map"] is None else "blocked", [precedent_blocker_id]))
