...
bindings. On the current default baseline those two artifact types are not
implemented. Their absence therefore creates explicit blockers and cannot be
treated as an empty successful review. A supplied future binding must have the
expected schema and study identity. Because this slice has neither owner
schema nor owner semantic/promotability validator, a supplied binding is
recorded only as `bound_unvalidated`; it retains an explicit gate blocker and
cannot clear scientific readiness. Presence or a self-consistent payload hash
is never evidence that support applies to this exact network or that a TS
precedent is promotable.
...
- reaction-literature owns literature evidence; mechanism-support and
  TS-precedent artifacts require their own future reviewed contracts;
