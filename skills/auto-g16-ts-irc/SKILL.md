...
12. After both IRC directions complete, run `audit-irc-endpoint` separately on each final point, then create one immutable `gaussian-endpoint-structure-review/1` per direction with `build-endpoint-structure-review`. Each review binds the exact input, full log, result, job, checkpoint, endpoint coordinates and audit by relative path, SHA-256 and size; records parser version/schema, stable atom IDs, identity, connectivity, stereochemistry, reviewer, rationale and time; and replays the source audit. Chemical-side strings alone are insufficient.
    ...
    validator reconstructs both endpoint reviews and requires one reactant and
