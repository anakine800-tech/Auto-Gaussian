#!/usr/bin/env python3
"""The single legacy RTwin/PBS execution backend for Auto-G16 v2.6."""
...
from execution_models import (
    ...
    ModelError,
    PlanKind,
    TransportAuthorityReplay,
    RuntimeBinding,
    ValidatedAttestationOperation,
    ValidatedRuntimePlan,
    WorkspacePaths,
    WorkspacePolicy,
    _runtime_binding_from_adapter,
)
...
def parse_gaussian(path: Path) -> dict[str, Any]:
    ...
    if has_only_fixed_constraint_directives:
        report.update(
            {
                "fixed_constraint_directive_count": len(
                    fixed_constraint_directives
                ),
                "has_only_fixed_constraint_directives": True,
                "fixed_constraint_directives": fixed_constraint_directives,
            }
        )
...
def _stable_stage_source(path: Path, label: str) -> dict[str, Any]:
    """Capture one no-follow source into a private streaming snapshot."""
...
def plan_legacy_stage_bytes(
    *,
        ...
        captured.append(
            {
                ...
                "bytes": stable["bytes"],
                "private_snapshot": stable["private_snapshot"],
    ...
    input_sha256 = captured[0]["source_identity"]["sha256"]
    ...
    def artifact_sha256(item: dict[str, Any]) -> str:
        ...
        return hashlib.sha256(item["bytes"]).hexdigest()

    def artifact_size(item: dict[str, Any]) -> int:
        identity = item["source_identity"]
        if identity is not None:
    ...
    portable_artifacts = [
        {
            ...
            "order": index,
            "sha256": artifact_sha256(item),
            "size_bytes": artifact_size(item),
        }
...
class LegacyGaussianRuntimeAdapter:
    """Validate the closed legacy_stdin invocation before PBS rendering."""

    executable_token = "g16"

    def capabilities(self) -> tuple[str, ...]:
        return ("gaussian_legacy_stdin", "link0_exact_resources")
    ...
    def validate_binding(
        ...
        memory = input_audit.get("mem")
        if not isinstance(input_value, str) or not isinstance(input_sha256, str):
            raise ModelError("Gaussian audit lacks exact input identity")
        input_path = Path(input_value)
        if not input_path.is_absolute() and input_path.name != input_value:
            raise ModelError("Gaussian input identity must be an exact basename")
        if nproc != exact_resources.cores:
            raise ModelError("Gaussian %nprocshared differs from exact resources")
        if parse_memory(memory) != exact_resources.memory_gb * 1024**3:
            raise ModelError("Gaussian %mem differs from exact resources")
        binding = _runtime_binding_from_adapter(
        ...
        return ValidatedRuntimePlan(binding, workspace_paths)


class LegacySchedulerAdapter:
    """Own the fixed legacy PBS renderer; callers cannot provide script bytes."""

    template_version = "legacy-pbs-template/1"

    def render(self, plan: ValidatedRuntimePlan) -> bytes:
        plan.runtime_binding.assert_owner_sealed()
    ...
    def submit_once(self, plan: CommandPlan, authorization: object) -> subprocess.CompletedProcess:
        plan.assert_owner_sealed()
        if plan.kind is not PlanKind.MUTATION_ONCE:
            raise ModelError("scheduler submit requires a sealed one-shot mutation plan")
        if not getattr(authorization, "consumed", False):
            raise ModelError("scheduler submit requires atomically consumed authorization")
        return run(list(plan.argv), input_bytes=plan.stdin, check=False, timeout_seconds=plan.timeout_seconds)


class LegacyTransportAdapter:
    """Build only the fixed two-hop operations owned by the legacy backend."""

    def capabilities(self) -> tuple[str, ...]:
        return (
            "typed_identity_attestation", "two_hop_exact_transfer",
            "pbs_inspect_exact", "pbs_fetch_allowlist",
        )

    def attest_first_hop_once(self, request: ValidatedAttestationOperation) -> AttestationBoundaryPlan:
        request.assert_owner_sealed()
        if request.operation != "attest_first_hop_once":
            raise ModelError("wrong typed operation for first-hop attestation")
        return AttestationBoundaryPlan.from_validated(request)

    def attest_nested_hop_once(
        self,
        first_hop: object,
        request: ValidatedAttestationOperation,
    ) -> AttestationBoundaryPlan:
        request.assert_owner_sealed()
        if request.operation != "attest_nested_hop_once":
            raise ModelError("wrong typed operation for nested attestation")
        if getattr(first_hop, "receipt_payload_sha256", None) != request.first_hop_receipt_sha256:
...
class LegacyRTWinPBSBackend:
    ...
    def __init__(self) -> None:
        ...
        self.runtime = LegacyGaussianRuntimeAdapter()
        self.scheduler = LegacySchedulerAdapter()
        self.transport = LegacyTransportAdapter()

    def capability_report(self) -> dict[str, Any]:
        return {
            "backend_kind": self.backend_kind,
            "transport": self.transport.capabilities(),
            "runtime": self.runtime.capabilities(),
            "scheduler_template_version": self.scheduler.template_version,
            "fixed_remote_root": DEFAULT_REMOTE_ROOT,
            "automatic_mutation_retry": False,
            "delete_supported": False,
        }

class LegacyCLICompatibilityAdapter:
    """Validate the frozen CLI surface before calling the typed backend."""
    ...
    def dispatch(self, args: argparse.Namespace) -> None:
        ...
        # flag, environment value, or import path selects another engine.
        if args.func in {_legacy_transaction_once, command_submit}:
            if args.dry_run:
                _legacy_transaction_once(args, _transaction_token=_BACKEND_TRANSACTION_TOKEN)
                return
            self._submit_new(args)
            return
        args.func(args)

    def _run_offline_differential_transaction(self, args: argparse.Namespace) -> None:
        """Test-only old/new command-plan differential; never a CLI selector."""

        _legacy_transaction_once(args, _transaction_token=_BACKEND_TRANSACTION_TOKEN)


def command_submit(args: argparse.Namespace) -> None:
    """Compatibility action that always dispatches through the sole backend."""

    LegacyCLICompatibilityAdapter().dispatch(args)
