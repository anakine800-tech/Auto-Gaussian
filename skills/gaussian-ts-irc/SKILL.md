...
8. Build coordinate-free retry inputs with `build-allcheck-irc`. Require `Geom=AllCheck Guess=Read RCFC`, an explicit direction, a passed checkpoint audit, and no title/charge/coordinates after the route. The builder refuses `ReCorrect=Never`.
