# Auto-G16 Metal TS P2-P4 Runtime Contract
...
reviewed range, and a displacement assessment bound to the result hash. The
assessment must confirm the intended coordinate and reject unintended
coordination or ligand-loss motion.

Promotion is a separate explicit decision. Only an accepted result can be
promoted; rejection and blocking are preserved. None of the three artifacts
authorizes input rendering, SSH, PBS, Gaussian, submission, retry,
cancellation, cleanup, deployment, or main-group metal execution.

M3 adversarial validation must cover wrong charge/multiplicity/atom order,
wrong state, missing stability, spin contamination, coordination or ligand
loss, wrong imaginary mode, cross-state checkpoint/Hessian reuse, incomplete
basis/ECP coverage, and semantic mutation followed by payload rehashing.
