...
_PREDECESSOR_OWNER_NAMES = (
    "protected_submit_contract",
    "local_state_binding",
)
_OWNER_READ_CHUNK_SIZE = 64 * 1024
_MAX_OWNER_SOURCE_BYTES = 2 * 1024 * 1024
_OWNER_STAT_FIELDS = (
    "st_dev",
    "st_ino",
    "st_uid",
...
class _OwnerFileSnapshot:
    """Private stable bytes and full identity of one predecessor owner."""
...
class SealedProtectedInvocationBundle:
    ...
    _predecessor_owner_snapshots: tuple[_OwnerFileSnapshot, ...]
    ...
    def _from_owner(
        ...
        predecessor_owner_snapshots: tuple[_OwnerFileSnapshot, ...],
        stage_plan: SealedLegacyStagePlan,
        *,
        testing: bool,
        token: object,
        ...
        for name, item in {
            ...
            "_predecessor_owner_snapshots": predecessor_owner_snapshots,
    ...
    def assert_owner_sealed(self) -> None:
        ...
        _assert_owner_snapshot_integrity(
            self._predecessor_owner_snapshots
        )
    ...
    def assert_current(self) -> "SealedProtectedInvocationBundle":
        ...
        self.assert_owner_sealed()
        current_owner_snapshots = _capture_predecessor_owner_snapshots()
        if current_owner_snapshots != self._predecessor_owner_snapshots:
            raise ProtectedInvocationError(
                "protected invocation predecessor owner identity differs"
            )
        current = _utc_now()
        if (
        ...
        with _exact_adjacent(
            "protected_submit_contract",
            snapshot=_owner_snapshot(
                current_owner_snapshots,
                "protected_submit_contract",
            ),
        ) as protected:
            ...
            with _exact_adjacent(
                ...
                try:
                    with _bind_local_protected_owner(
                        local,
                        protected,
                    ):
                        if self._testing:
        ...
        with _bind_sealed_local_protected_owner(
            self.local_state_binding,
            protected,
        ):
            self.local_state_binding.assert_current()
        _assert_stage_plan_current(
            self.stage_plan,
            protected_owner_snapshot=_owner_snapshot(
                current_owner_snapshots,
                "protected_submit_contract",
            ),
        )
        if (
            _capture_predecessor_owner_snapshots()
            != self._predecessor_owner_snapshots
        ):
            raise ProtectedInvocationError(
                "protected invocation predecessor owner identity differs"
...
def _adjacent_path(name: str) -> Path:
    if name not in _PREDECESSOR_OWNER_NAMES:
        raise ImportError(f"unsupported adjacent owner: {name}")
...
def _stat_identity(info: os.stat_result) -> tuple[int, ...]:
    return tuple(getattr(info, field_name) for field_name in _OWNER_STAT_FIELDS)


def _stable_owner_snapshot(name: str) -> _OwnerFileSnapshot:
    ...
    try:
        ...
        if stat.S_ISLNK(before.st_mode) or not stat.S_ISREG(before.st_mode):
            raise ProtectedInvocationError(
                f"{name} owner must be a no-follow regular file"
            )
        descriptor = os.open(
            path,
            os.O_RDONLY
        ...
        if _stat_identity(opened) != _stat_identity(before):
            raise ProtectedInvocationError(
                f"{name} owner changed while opening"
            )
        if (
            opened.st_size < 1
            ...
            raise ProtectedInvocationError(
                f"{name} owner size is outside the fixed bound"
            )
        chunks = []
        while True:
        ...
        if isinstance(exc, ProtectedInvocationError):
            raise
        raise ProtectedInvocationError(
            f"{name} owner stable read failed closed: {exc}"
        ) from exc
    finally:
        if descriptor >= 0:
            os.close(descriptor)
    ...
    if len(set(identities)) != 1:
        raise ProtectedInvocationError(
            f"{name} owner identity drifted during stable read"
        )
    source_bytes = b"".join(chunks)
    if len(source_bytes) != opened.st_size:
        raise ProtectedInvocationError(
            f"{name} owner stable read was short"
        )
    canonical_path = path.resolve(strict=True)
    if canonical_path != path or canonical_path.parent != Path(
        __file__
    ).resolve(strict=True).parent:
        raise ProtectedInvocationError(
            f"{name} owner canonical path differs"
        )
...
def _capture_predecessor_owner_snapshots(
) -> tuple[_OwnerFileSnapshot, ...]:
    return tuple(
        _stable_owner_snapshot(name)
        for name in _PREDECESSOR_OWNER_NAMES
    )


def _owner_snapshot(
    snapshots: tuple[_OwnerFileSnapshot, ...],
    name: str,
) -> _OwnerFileSnapshot:
    matches = tuple(
        snapshot for snapshot in snapshots if snapshot.name == name
    )
    if len(matches) != 1:
        raise ProtectedInvocationError(
            f"{name} owner snapshot is unavailable"
        )
    return matches[0]


def _assert_owner_snapshot_integrity(
    snapshots: tuple[_OwnerFileSnapshot, ...],
) -> None:
    if tuple(snapshot.name for snapshot in snapshots) != (
        _PREDECESSOR_OWNER_NAMES
    ):
        raise ProtectedInvocationError(
            "protected invocation predecessor owner set differs"
        )
    for snapshot in snapshots:
        if (
            snapshot.canonical_path != _adjacent_path(snapshot.name)
            or snapshot.size != len(snapshot.source_bytes)
            or hashlib.sha256(snapshot.source_bytes).hexdigest()
            != snapshot.sha256
        ):
            raise ProtectedInvocationError(
                f"{snapshot.name} owner sealed snapshot differs"
            )
...
def _exact_adjacent(
    ...
    *,
    ...
    if snapshot is None:
        snapshot = _stable_owner_snapshot(name)
    path = _adjacent_path(name)
    if snapshot.name != name or snapshot.canonical_path != path:
        raise ImportError(f"{name} owner snapshot origin differs")
    if (
        snapshot.size != len(snapshot.source_bytes)
        or hashlib.sha256(snapshot.source_bytes).hexdigest()
        != snapshot.sha256
    ):
        raise ImportError(f"{name} owner snapshot bytes differ")
    code = compile(
...
@contextlib.contextmanager
def _bind_local_protected_namespace(
    namespace: dict[str, Any],
    protected: types.ModuleType,
) -> Iterator[None]:
    original = namespace.get("_exact_protected_contract")
    if not callable(original):
        raise ImportError(
            "local-state predecessor owner loader is unavailable"
        )

    @contextlib.contextmanager
    def exact_captured_protected() -> Iterator[types.ModuleType]:
        yield protected

    with _MODULE_LOCK:
        namespace["_exact_protected_contract"] = exact_captured_protected
        try:
            yield
        finally:
            namespace["_exact_protected_contract"] = original


@contextlib.contextmanager
def _bind_local_protected_owner(
    local: types.ModuleType,
    protected: types.ModuleType,
) -> Iterator[None]:
    with _bind_local_protected_namespace(
        local.__dict__,
        protected,
    ):
        yield
...
@contextlib.contextmanager
def _bind_sealed_local_protected_owner(
    binding: object,
    protected: types.ModuleType,
) -> Iterator[None]:
    method = getattr(type(binding), "assert_current", None)
    namespace = getattr(method, "__globals__", None)
    if not isinstance(namespace, dict):
        raise ImportError(
            "sealed local-state predecessor owner namespace is unavailable"
        )
    with _bind_local_protected_namespace(namespace, protected):
        yield
...
def _assert_stage_plan_current(
    plan: SealedLegacyStagePlan,
    *,
    protected_owner_snapshot: _OwnerFileSnapshot | None = None,
) -> None:
    with _exact_adjacent(
        "protected_submit_contract",
        snapshot=protected_owner_snapshot,
    ) as protected:
        with protected._skill_owner_graph() as modules:
            legacy = modules["gaussian_rtwin_pbs"]
            try:
...
class ProtectedInvocationContractOwner:
    ...
    def seal(
        ...
        if (
            ...
            raise ProtectedInvocationError(
                "protected invocation predecessor owner identity drifted "
                "during sealing"
            )
