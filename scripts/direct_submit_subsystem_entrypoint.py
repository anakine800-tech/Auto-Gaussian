#!/usr/bin/env python3
"""Fixed production submit sshd Subsystem entrypoint."""
...
_REGISTRY_KEY = "_auto_g16_direct_submit_subsystem_entrypoint_v1"
if getattr(sys, _REGISTRY_KEY, None) is not None:
    raise ImportError("submit subsystem entrypoint was reloaded")
_ENTRY_TOKEN = object()
...
def main() -> int:
    if (sys.argv[1:] != [] or globals().get("_FROZEN_MAIN") is not main
        ...
        raise ImportError("submit subsystem argv is closed")
    module = _load_reviewed_bootstrap()
    return module.submit_main()
