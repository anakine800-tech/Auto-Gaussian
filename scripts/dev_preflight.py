...
def inspect(root: Path, environment: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    if entries:
        checks.append(
            _check(
                "working_tree",
                "warning",
                "working tree is dirty; classify and carry only changes owned by this task",
                staged=staged,
                unstaged=unstaged,
                untracked=untracked,
                total=len(entries),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = inspect(find_git_root(args.repo))
