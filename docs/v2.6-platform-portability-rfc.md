...
状态：Phase 0，待仓库所有者决策；不构成实现或运行授权<br>
...
P0/P1、required check 失败、golden 未解释变化、owner 三项决策未完成、敏感内容、dirty
...
- 本 RFC 未获 owner 批准，停止于 Phase 0；
- 三项 owner decision 任一未决，相关实现 PR 不开始或不合并；
...
1. **版本基线**：是否规定每个 v2.6 实现 PR 从当时刷新并确认的 `main` 开始，同时以
   `v2.5.4^{}` 的 `aa1c4614...` 作为 legacy golden 原点？<br>
   **推荐默认：是。** v2.5.4 已发布；不再把发布状态当 blocker，但后续 PR 仍重新核实 main。

2. **remote-root 政策**：是否批准“legacy 永久固定 SDL；新 backend 使用 mandatory、
   backend-owned、reviewed、hash-bound、不可 CLI/env 覆盖的 root”，并在实现前单独修改
   `AGENTS.md`？<br>
   **推荐默认：有条件批准选项 B。** owner 未完成规则变更时使用选项 A，所有 backend 仍固定
   `/home/user100/SDL`。

3. **v2.6 支持边界**：是否只支持 `legacy_rtwin_pbs` + `direct_ssh_pbs` + offline onboarding，
   将 local Gaussian、Slurm 和 MCP 明确标为后续版本 unsupported？<br>
   **推荐默认：是。** 先证明两个 PBS backend 共享的最小合同，再分别扩展。

## 19. 推荐后续任务序列

Owner 先回答第 18 节三项；随后依次创建第 12 节 PR1–PR6。任何 live smoke 另建 operational
task 并使用 owning Skill 的 exact approval，不与 feature PR 合并授权。Phase 0 到此停止，不
进入代码或 schema 实现。
