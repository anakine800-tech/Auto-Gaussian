...
TransportIdentityBinding digest 全部进入 profile self-hash。两项缺失、互换、相同或 role/ref
混淆均 fail closed。`/1` 不自动迁移、rehash 或 backfill 为 `/2`。
...
4. 只有 receipt 完整、一致且在时间窗内，才允许执行固定
   ...
   `mutation_allowed=false`，并产生 hash-only `NestedHopIdentityHandshakeReceipt`；receipt 绑定
   profile、identity binding、两个 config-ref binding、Stage A/Stage B receipt、第二跳 identity、
   observed-fingerprint evidence、nonce/time/version 与自身摘要。实际 fingerprint 再与批准
   evidence 比较。三个 read-only operation 均一致后才可进入另一个既有 mutation authority。
...
PR4B clean-stop 后的最小兼容演进使用 `auto-g16-execution-authorization/2` overlay。它精确引用
并 replay `/1` request/authorization 与 historical profile `/1`，同时绑定语义等价的 profile `/2`、
...
- PR4B 前置补丁新增 `auto-g16-execution-profile/2`、
  `auto-g16-execution-authorization/2` overlay 与
  `auto-g16-nested-hop-identity-handshake-{request,receipt}/1`。既有 `/1` Schema bytes/owner
