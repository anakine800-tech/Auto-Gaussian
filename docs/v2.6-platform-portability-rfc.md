...
复审关闭既有 P1/P2 后才可作为实现合同；不构成实现或运行授权<br>
...
`/home/user100/SDL` 政策。仓库所有者虽已条件批准第 9 节选项 B，仍须在独立 PR5 中亲自审阅
并修改绑定规则后才能实施；在此之前，所有 backend 都必须继续使用固定根。
...
ExecutionProfile 只保存完整 binding 的摘要；authorizing successor 同时绑定该摘要。每次 live
operation 在建立连接前重新 no-follow 读取全部 source、解析 alias，并重算 binding；SSH handshake
只允许核对 host-key evidence，不得先执行远程命令或传输。第一次 network mutation 前必须再次
比较原批准摘要、刚重算摘要和实际 handshake fingerprint；任一漂移立即 fail closed，旧批准失效。
...
TransportIdentityBinding、workspace、RuntimeBinding、resource policy/gate、scheduler snapshot、
execution batch、one-time live approval 和 idempotency binding。backend 不得改写上游结论。

## 5. 非授权 Execution Request 与授权闭包

`auto-g16-execution-request/1` 只表达 intent：input hash、task/attempt、work kind、profile ID/hash、
required capabilities、proposed exact resources 和上游 artifact refs。它必须 non-authorizing。
...
该 successor 的合同名固定为 `auto-g16-execution-authorization/1`，由 PR3 创建 owner schema 与
validator；本 Phase 0 不创建 schema。旧 live approval、batch、gate 和 job artifact 均按原
validator replay；不得“补写” profile/transport hash 或重新 hash 冒充新合同。新 direct backend
绝不接受 legacy-only approval。PR4 切换 legacy 新提交后也必须使用该 successor；CLI 参数名可以
保持不变，由 validator 按 schema dispatch。
...
在 2.6.x 兼容期继续有效；映射后的 alias、两份 config 的完整 source-bundle digest、两跳 effective
target identity digest 和适用 host-key evidence 共同生成 TransportIdentityBinding，再进入 derived
profile hash。只 hash config path/alias reference 不合格；无法完整解析任一 hop 时 live fail closed。
不得新增 remote-root CLI/env override。
...
仓库所有者已条件批准 B，但 `AGENTS.md` 尚未按 PR5 修改，因此当前实际政策仍为 A，direct
backend 只能使用 SDL。B 的 root 必须是 backend-owned policy 的安全字段，不能是临时 CLI、
环境变量或“advanced override”；批准记录必须绑定 profile hash 和 canonical root evidence。
C 可用于少数共享受控站点，不作为公共用户默认。

## 10. Schema、versioning 与 packaging

- 冻结 `auto-g16-runtime-config/1`；PR2 新增 `auto-g16-execution-profile/1`、
  `auto-g16-transport-identity-binding/1`、`auto-g16-resource-catalog/1` 和
...
| SSH config/alias | 旧选择表面保留；exact config source bytes、alias、effective target 和 host-key evidence 生成不可变 binding | 只引用 path/alias、source/target drift 或 fingerprint mismatch 时拒绝 live；report 只公开摘要 |
...
| --- | --- | --- | --- |
| PR1 | `codex/v26-legacy-golden`：只增加 legacy characterization fixtures/tests；不重构生产路径 | **L2**；independent compatibility reviewer + RTwin/PBS domain reviewer | 本 RFC independent L2 复审关闭 P1/P2 |
| PR2 | `codex/v26-platform-contracts`：完整 profile、TransportIdentityBinding、CapabilityReport、ResourceCatalog shape、legacy catalog validation、canonical hash、offline init/validate/doctor 与 legacy mapping | **L3**（target identity/config security boundary）；repository owner + transport/security domain owner | PR1 已合并且 golden 原点冻结 |
...
2. focused：strict profile canonical projection/self-hash、TransportIdentityBinding source/alias/target/
   host-key drift、CapabilityReport redaction、ResourceCatalog exact legacy shape、RuntimeBinding、
   WorkspacePaths、fixed renderer 和 authorization successor。
3. 对抗：duplicate/unknown/NaN/floating/exponent/BOM/malformed UTF-8/canonical key order/terminal LF/
   self-hash forgery；config Include/content/alias/effective target/host-key drift；symlink/traversal；
...
先提供 offline doctor 展示 derived legacy profile ID/hash、transport identity 摘要、catalog 和
capabilities，不写旧配置或公开 identity 原值。用户可显式生成新 profile，review diff 后切换。
冲突时停止并给字段级提示。历史项目继续由 legacy validator/adapter 管理；不批量迁移 artifact。
...
- 对本修订的 independent L2 复审尚未关闭原 P1/P2，停止把 RFC 当作实现合同；
...
第 18 节决议已完成。本修订先由 independent L2 reviewer 确认原 5 个 P1、2 个 P2 均已关闭；
...
### 20.2 本修订 final-tree 证据合同

把当前 tree SHA 写进同一 tree 会形成自引用，因此 exact final tree SHA、final commit SHA、UTC、
exit/totals/duration/modifiers 记录在该 commit 的 evidence trailers 与任务最终 handoff；RFC 以
`FINAL_RFC_COMMIT^{tree}` 指向同一 immutable tree。V3–V6 可在 commit 前对已 staged、无 unstaged
diff 且已记录 exact `git write-tree` 的 frozen candidate 执行；V1–V3/V6 在 commit 后再做 clean-tree
确认。仅创建 commit 不改变 tree。若文件 bytes 再变，全部 V-row 失效。

| ID | 绑定对象 | exact command | 必须记录的 result |
| --- | --- | --- | --- |
| V1 | `FINAL_RFC_COMMIT^{tree}` | `./scripts/python core scripts/dev_preflight.py --require-clean` | UTC、exit、branch、HEAD/tree、clean/linked-worktree、profile/modifiers |
| V2 | 同上 | `git diff --check a73ea2922171d0482dabfe09fad1b5ac5aa03a5c..FINAL_RFC_COMMIT -- docs/v2.6-platform-portability-rfc.md` | UTC、exit、唯一 changed path |
...
| V6 | 同上 | 下方 `V6 exact command`，另人工 review `git show --stat --oneline FINAL_RFC_COMMIT` 与 `git show --format=fuller -- docs/v2.6-platform-portability-rfc.md` | UTC、rg exit 1 = 0 hits、scope/pattern class；人工确认唯一文件 |
...
V1–V6 是本修订必要且充分的本地证据；它们不实现第 14 节未来 contract/adversarial tests，也不
证明 remote CI、branch protection、source-archive release readiness、live compatibility、L2/L3
review closure 或科学接受。本 Phase 0 明确没有 source-archive replay、SSH、RTwin、PBS、Gaussian、
live smoke、deployment、push、PR、merge、tag 或 release。
