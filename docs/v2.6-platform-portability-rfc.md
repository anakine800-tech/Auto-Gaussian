...
## 0. Release-scope revision — 2026-07-31

The repository owner has frozen a narrower v2.6.0 release scope than the
original comprehensive gate in section 17. Version 2.6.0 closes only the
offline foundation that already has local independent L3 evidence. PR4 real
production adapter wiring and provider, PR6 direct SSH/PBS, dependent
onboarding/runtime claims, and every live smoke are deferred to 2.6.1.

This revision changes release scope only. It does not revise, waive, or weaken
the original implementation, authority, identity, server-safety, scientific,
review, or live-smoke gates. The original comprehensive v2.6.0 gate remains
verbatim in section 17 as historical design provenance, but it is superseded
as a 2.6.0 release criterion by this revision. Any deferred capability must
satisfy its original applicable gates before a later release may claim it.
...
observe_fresh_root_once(
  stable_evidence,
  authorization_scope
) -> FreshRootObservationCapability {
...
新 backend 的 workspace-policy owner 必须把 root 合同拆成两个对象：

1. closed、immutable、hash-bound `StableRootIdentityEvidence` 只绑定 backend/owner、profile ID/hash、
   reviewed root policy、exact declared root、expected canonical root/existing-component identity、
   path-normalization/containment semantics、project/scratch derivation grammar、fresh/no-overwrite/
   no-symlink/no-delete markers和自身 payload hash；它明确排除 observation time、expiry、nonce 与
   receipt ID。exact human approval 同时绑定 profile hash、stable evidence hash、fresh-observation
   operation version/max age/single-consumption requirement；只批准 root 字符串或预绑定未来 receipt
   hash 均不合格。
2. 首次 remote mutation 前，owner 从稳定 no-follow observation 产生 time-bounded
   `FreshRootObservationReceipt`。receipt 引用 approved stable evidence hash，并绑定 authorization
   scope、operation/task/attempt、nonce、observed/expires time、实际 component identities、
   project/scratch containment/freshness、comparison classification 与自身 hash。receipt 本身
   non-authorizing；owner 同时发行绑定相同 component identities 的 non-copyable single-use
   descriptor/capability。

mutation owner 必须在同一原子 owner operation 内或通过 descriptor-relative no-follow operation
消费该 exact capability，不得在检查后按 path reopen/re-resolve/substitute。missing/partial/expired/
replayed/unknown receipt、profile/stable evidence/component drift、symlink/reparse point、containment/
freshness drift、path reopen 或 capability failure 均必须在 reservation/claim/write/transfer/任何
remote effect 前停止；不能证明 check-to-use closure 时 fail closed。
...
stable root-identity evidence hash，fresh observation receipt 在 mutation time 引用该 stable hash 并
通过 single-use descriptor/capability 闭合 check-to-use。C 可用于少数共享受控站点，不作为公共
用户默认。
...
- remote-root owner decision 已写入仓库策略；任何新 backend 仍须 additive profile/stable-root-
  evidence/fresh-observation/authorization 合同、descriptor/capability-bound mutation owner 与
  owner validator 闭合，否则只允许 SDL。
...
以下是本 RFC 原始、全面的 `v2.6.0` release gate。为保留历史可追溯性而原文保留；根据第 0 节
2026-07-31 范围修订，它已不再是缩减后 v2.6.0 offline-foundation 候选的 release criterion，
而继续约束任何未来声称相应延期能力的版本：PR1–PR4、PR6、PR7 均合并；若宣称非 SDL 支持，PR5 也已合并；
...
- PR5 仓库策略本身不授权 non-SDL；新 backend 的 additive profile/stable-root-evidence/
  fresh-observation/authorization 合同、descriptor/capability-bound check-to-use、owner validator、
  adversarial tests 或 L3 review 任一未闭合，任何非 SDL profile 拒绝；
