...
4. 只有 receipt 完整、一致且在时间窗内，才允许完成第二跳 host-key-only/identity handshake；
   实际 fingerprint 再与批准 evidence 比较。两个 hop 均一致后才开放第一次 network mutation。
...
批准 binding 比较 → 第二跳 handshake；两个 hop 均一致后，才可在第一次 network mutation 前重放
