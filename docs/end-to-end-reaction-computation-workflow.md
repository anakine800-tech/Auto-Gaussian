...
| --- | --- | --- |
| ChemDraw molecule reconstruction and stereochemical review | implemented in W0 feature source | the richer strict reaction-package workflow has been recovered from the installed copy; a separately approved real native-ChemDraw smoke test is still required before integration |
...
| reaction intake/species registry/stoichiometric balance foundation | implemented offline on the W0/W1 feature branch | stable atom identities and exact occurrence binding are implemented; cross-state atom-map proposal belongs to W3 and remains missing |
| condition-to-model mapping | implemented offline on the W0/W1 feature branch | every transcribed condition requires one reviewed treatment; no solvent, additive, counterion or standard-state model is inferred |
...
dry-run diff review; all seven repository/deployed Skill pairs now match.
...
and blocked catalytic intake fixture are implemented. The remaining W1
integration gate is one real strict ChemDraw reaction package, preferably the
CAT2 case when its exact source file is available, without manufacturing its
active catalyst, atom map, protocol or candidate ensemble.
