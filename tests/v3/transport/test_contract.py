...
class TransportContractTests(unittest.TestCase):
    ...
    def test_rtwin_launcher_is_exact_and_narrow(self) -> None:
        ...
        self.assertIn("$InputTask=$Process.StandardInput.BaseStream.WriteAsync($RequestFrame,0,$RequestFrame.Length)",_RTWIN_LAUNCHER_SOURCE)
        self.assertNotIn("BaseStream.Write($RequestFrame",_RTWIN_LAUNCHER_SOURCE)
        ...
        self.assertLess(_RTWIN_LAUNCHER_SOURCE.index("$OutputTask=$Process.StandardOutput.BaseStream.ReadAsync"),_RTWIN_LAUNCHER_SOURCE.index("$InputTask=$Process.StandardInput.BaseStream.WriteAsync"))
        self.assertLess(_RTWIN_LAUNCHER_SOURCE.index("$ErrorTask=$Process.StandardError.BaseStream.ReadAsync"),_RTWIN_LAUNCHER_SOURCE.index("$InputTask=$Process.StandardInput.BaseStream.WriteAsync"))
        self.assertLess(_RTWIN_LAUNCHER_SOURCE.index("$InputTask=$Process.StandardInput.BaseStream.WriteAsync"),_RTWIN_LAUNCHER_SOURCE.index("$Process.StandardInput.Close()"))
        self.assertIn("if($InputOpen){$Pending.Add($InputTask)}",_RTWIN_LAUNCHER_SOURCE)
    ...
    def test_launcher_duplex_pump_drains_before_finite_async_input(self) -> None:
        pump = _RTWIN_LAUNCHER_SOURCE.split("if(-not $Process.Start()){Fail-AutoG16}", 1)[1]
        output_start = pump.index("$OutputTask=$Process.StandardOutput.BaseStream.ReadAsync")
        error_start = pump.index("$ErrorTask=$Process.StandardError.BaseStream.ReadAsync")
        input_start = pump.index("$InputTask=$Process.StandardInput.BaseStream.WriteAsync")
        wait_start = pump.index("[Threading.Tasks.Task]::WaitAny")
        input_close = pump.index("$Process.StandardInput.Close()")
        self.assertLess(output_start, input_start)
        self.assertLess(error_start, input_start)
        self.assertLess(input_start, wait_start)
        self.assertLess(wait_start, input_close)
        self.assertIn("if($InputOpen){$Pending.Add($InputTask)}", pump)
        self.assertIn("if($OutputOpen){$Pending.Add($OutputTask)}", pump)
