...
def _freeze(value: object, path: str, active: set[int] | None = None) -> object:
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        ...
        try:
            return tuple(
                _freeze(item, f"{path}[{index}]", containers)
...
class ThermodynamicEnsemble:
    """One immutable, complete aggregation of the exact eligible member set."""
    ...
    conformer_ensemble_revision: int
    ...
    functional_kernel_implementation_id: str
    functional_kernel_implementation_binding: Mapping[str, object]
    ...
    def _create(cls, **fields: object) -> ThermodynamicEnsemble:
        ...
        for name in (
            ...
            "conformer_ensemble_revision",
            "source_member_ids",
            "temperature_k",
            "standard_state",
            "thermochemistry_policy_id",
            "thermochemistry_policy_payload_sha256",
            "functional_kernel_implementation_id",
        ...
        for name in (
            ...
            "functional_kernel_implementation_binding",
    ...
    def _identity_payload(self) -> Mapping[str, object]:
        return {
            ...
            if name not in {"thermodynamic_ensemble_id", "payload_sha256"}
        }
